export const APP_VERSION = '2.11.66';
export const VERSION = '2.11.66';
export const APP_VERSION_DATE = '08/07/2026 22:50';
export const CHANGELOG = [
  ...
  '',
  '## [V2.11.16] - 23/05/2026 21:52',
  '### Multi-Avatar — 3 avatares por e-mail com nick único global',
  '- **3 slots por conta**: cada e-mail suporta até 3 avatares independentes (saves separados no Firestore: `saves/{uid}`, `saves/{uid}_s2`, `saves/{uid}_s3`).',
  '- **Nick = Nome do treinador, único global**: ao criar um avatar, o nick é reservado atomicamente via transação no Firestore (`nicknames/{nick_lower}`). Nenhum duplicado é permitido no mundo inteiro.',
  '- **Tela de seleção de avatar**: aparece automaticamente ao logar quando a conta tem 2 ou 3 avatares. Cada slot mostra o treinador com data de criação.',
  '- **Criar avatar**: botão por slot vazio com campo de nick e verificação de disponibilidade em tempo real (debounce 600ms).',
  '- **Deletar avatar**: confirmação dupla; apaga save, perfil público e libera o nick.',
  '- **Perfis públicos separados**: cada slot tem seu próprio documento em `users/{docId}` para ranking e amigos.',
  '- **Save local por slot**: `poke_idle_save_{uid}` (slot 1, backward compat) e `poke_idle_save_{uid}_s2/s3`.',
  '- **Backward compat total**: contas existentes continuam funcionando como slot 1 sem qualquer migração forçada.',
  '',
  '## [V2.11.15] - 23/05/2026 21:41',
  '### Regional Lock — Correção de bloqueio de Pokémon por região',
  '- **Bug de migration corrigido**: Pokémon antigos sem `capturedRegion` agora recebem a região pelo Pokédex Nacional (`getPokemonRegion`), em vez de `kanto` hardcoded. Ex: Meganium (#154) recebe `capturedRegion=johto` corretamente.',
  '- **Regra de campeão restrita**: ser Campeão da região atual libera apenas as regiões **anteriores** na ordem regional; Pokémon de regiões futuras permanecem bloqueados mesmo após a liga.',
  '- **Visual melhorado no PC**: Pokémon bloqueados exibem cadeado vermelho (regional) ou laranja (nível), label da região de origem, e tooltip explicativo ("Capturado em Johto — derrote a Liga de Hoenn para desbloquear").',
  '- **Mensagem de erro específica**: ao tentar mover Pokémon bloqueado para o time, o modal explica exatamente o motivo e o que fazer para desbloqueá-lo.',
  '',
  '## [V2.11.14] - 23/05/2026 21:31',
  '### Sync - Resolução de Conflito de Save',
  '- **Detecção de conflito**: ao fazer login, compara o timestamp do save da nuvem (`updatedAtClient`) com o do localStorage deste dispositivo (`savedAt`).',
  '- **Modal de conflito**: se o save local for mais de 5 minutos mais recente que a nuvem, exibe o `SaveConflictModal` mostrando data, insígnias, Pokémon capturados e tamanho do time de cada save.',
  '- **Escolha explícita**: o jogador decide qual save usar; o save escolhido é sincronizado imediatamente com a nuvem.',
  '- **`loadGameState` retorna metadados**: agora retorna `{ gameState, cloudSavedAt }` em vez de só o estado, expondo o timestamp do Firestore para comparação.',
  '',
  '## [V2.11.13] - 23/05/2026 21:17',
  '### Battle Tower - Pool do Jogador + Isolamento de Time + Modal Shop',
  '- **Pool do PC**: draft e recrutas da shop agora usam apenas os Pokémon capturados pelo jogador (team + PC); fallback para básicos do POKEDEX se o PC estiver vazio.',
  '- **Isolamento total de time**: a Battle Tower não altera mais `gameState.team`; o time da run vive exclusivamente em `tower.activeRun.team`, preservando o time principal.',
  '- **Seleção de 4 golpes**: ao escolher um Pokémon com mais de 4 moves aprendidos, o jogador seleciona exatamente quais 4 levar para a batalha (fase `move_selection`).',
  '- **Modal de Shop imersivo**: após cada vitória o shop aparece como overlay dedicado, com título de andar, saldo visível e botão para prosseguir.',
  '- **Background de batalha**: arena com gradiente dinâmico baseado no tipo do inimigo ativo, grade sutil e glassmorphism nos painéis.',
  '- **Limpeza de código morto**: bloco legado de vitória da Torre em `handleBattleTick` removido (batalhas vão para `tower_battle`, não `battles`).',
  '',
