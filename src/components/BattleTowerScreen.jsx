...
import { getTowerStarters, startTowerRun, generateTowerEncounter, getRandomBoons, resumeTowerRun } from '../utils/towerLogic';
...
const BattleTowerScreen = ({ gameState, setGameState, setCurrentView, setCurrentEnemy, onOpenTowerCombat }) => {
  ...
  const handleSelectDraft = (pokemonInfo) => {
    ...
    setGameState(prev => ({
      ...prev,
      team: newRun.team,
      inventory: newRun.inventory,
      currentRoute: 'tower', // Para evitar conflitos de mapa
      tower: { 
        ...prev.tower, 
        activeRun: newRun,
        realTeam: prev.team,
  ...
  const handleBattle = () => {
    // Inicia combate da torre
    const encounter = generateTowerEncounter(tower.activeRun.floor);
    // Aqui nós precisaremos configurar o combat manager para usar o time da torre
    // Em AppRoot.jsx precisamos interceptar quando for 'tower_battle'
    ...
    return (
      <div className="h-full flex flex-col bg-slate-950 animate-fadeIn p-6 overflow-y-auto">
        <div className="flex justify-between items-center mb-8">
          ...
          <button onClick={() => setCurrentView('city')} className="text-white/40 hover:text-white font-black text-lg w-8 h-8 flex items-center justify-center">x</button>
        </div>
        ...
        <div className="flex flex-col gap-6 flex-1">
          ...
          <div className="bg-slate-900 border border-white/10 rounded-3xl p-6">
            <div className="flex justify-between items-center mb-4">
               ...
               <p className="text-yellow-400 font-black text-xl">{tower.bp.toLocaleString()} BP</p>
            </div>
          ...
          {draftOptions.map((poke, i) => {
            ...
            return (
              ...
              >
                ...
                <img src={`https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/${data.id}.png`} className="w-16 h-16 drop-shadow-lg z-10" alt={data.name} />
  ...
  useEffect(() => {
    if (run?.shopPending) {
      import('../utils/towerLogic').then(m => {
        setGameState(prev => {
          if (!prev.tower?.activeRun?.shopPending) return prev;
          const newRun = { 
            ...prev.tower.activeRun, 
            shopPending: false, 
            shop: m.generateFloorShop(prev.tower.activeRun.floor) 
          };
          return { ...prev, inventory: newRun.inventory, tower: { ...prev.tower, activeRun: newRun } };
        });
      });
    }
  ...
  return (
    <div className="h-full flex flex-col bg-slate-950 p-6 animate-fadeIn">
      ...
      <div className="grid grid-cols-2 gap-3 mb-6">
        {run.team.map((poke, i) => {
          const data = POKEDEX[poke.id];
          const hpPct = Math.max(0, (poke.currentHp / poke.maxHp) * 100);
          ...
          return (
            <div key={i} className={`bg-slate-900 border border-white/10 rounded-2xl p-3 flex flex-col ${isFainted ? 'opacity-50 grayscale' : ''}`}>
               <div className="flex justify-between items-start mb-2">
                 <img src={`https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/${data.id}.png`} className="w-10 h-10 object-contain drop-shadow-md" alt={data.name} />
               ...
               <div className="w-full h-1.5 bg-slate-800 rounded-full overflow-hidden">
                 <div className={`h-full ${hpPct > 50 ? 'bg-green-500' : hpPct > 20 ? 'bg-yellow-500' : 'bg-red-500'}`} style={{ width: `${hpPct}%` }} />
               </div>
               <p className="text-[8px] font-bold text-white/50 text-right mt-1">{Math.ceil(poke.currentHp)}/{poke.maxHp}</p>
            </div>
          );
        })}
      </div>
      ...
      <div className="bg-slate-900 border border-white/10 rounded-2xl p-4 mb-4">
         ...
         <div className="flex gap-2 mb-3">
            <div className="flex items-center gap-1.5 px-2 py-1 bg-white/5 rounded-lg border border-white/10">
               ...
               <span className="text-yellow-400 font-black text-xs">{run.inventory.coins}</span>
            </div>
            <div className="flex items-center gap-1.5 px-2 py-1 bg-white/5 rounded-lg border border-white/10">
               <span className="text-[10px] font-black uppercase text-white/50">Poções:</span>
               <span className="text-white font-black text-xs">{run.inventory.potions}</span>
            </div>
            <div className="flex items-center gap-1.5 px-2 py-1 bg-white/5 rounded-lg border border-white/10">
               <span className="text-[10px] font-black uppercase text-white/50">Revives:</span>
               <span className="text-white font-black text-xs">{run.inventory.revives}</span>
            </div>
         </div>

         <div className="grid grid-cols-1 gap-2">
           {run.shop && run.shop.map((offer, idx) => {
             ...
             const hasFainted = run.team.some(p => p.hp <= 0 || p.currentHp <= 0);
             const canAfford = run.inventory.coins >= offer.cost && !isTeamFull && !(isReviveBoon && !hasFainted);
             ...
             return (
               <button 
                 ...
                 onClick={() => {
                   ...
                   setGameState(prev => {
                      const st = { ...prev };
                      const newInv = { ...st.inventory, coins: st.inventory.coins - offer.cost };
                      const newTowerRun = { ...st.tower.activeRun, inventory: newInv, team: [...st.tower.activeRun.team] };
                      
                      if (offer.type === 'item') {
                         newInv[offer.id] = (newInv[offer.id] || 0) + 1;
                      } else if (offer.type === 'boon') {
                         if (offer.boon.id === 'revive_one') {
                            const faintedIdx = newTowerRun.team.findIndex(p => p.hp <= 0 || p.currentHp <= 0);
                            if (faintedIdx !== -1) {
                               newTowerRun.team[faintedIdx] = { ...newTowerRun.team[faintedIdx], hp: newTowerRun.team[faintedIdx].maxHp * 0.5, currentHp: newTowerRun.team[faintedIdx].maxHp * 0.5 };
                            }
                         } else {
                      ...
                      return { ...st, inventory: newInv, tower: { ...st.tower, activeRun: newTowerRun }, team: newTowerRun.team };
                   });
                 }}
                 disabled={!canAfford}
                 className={`flex items-center justify-between p-3 rounded-xl border text-left transition-all ${canAfford ? 'bg-slate-800 border-white/20 hover:bg-slate-700 hover:scale-[1.01] active:scale-[0.98]' : 'bg-slate-900 border-white/5 opacity-50 cursor-not-allowed'}`}
               >
                 <div>
                   <h5 className={`font-black text-xs uppercase ${offer.type === 'boon' ? 'text-amber-400' : offer.type === 'recruit' ? 'text-blue-400' : 'text-white'}`}>
