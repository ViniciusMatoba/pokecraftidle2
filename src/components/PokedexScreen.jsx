...
import { DEX_REWARDS, getDexRewardProgress, isDexRewardComplete, isDexRewardClaimed, claimDexReward, getDexProgressView } from '../data/dexRewards';
...
const PokedexScreen = ({ POKEDEX, caughtData, team = [], box = [], dexLimit = 151, routes = {}, gameState = {}, setGameState, onGoToRoute, onBack }) => {
  const [searchTerm, setSearchTerm] = useState('');
  const [selectedPoke, setSelectedPoke] = useState(null);
  const [tab, setTab] = useState('dex'); // 'dex' | 'progress'
    ...
    <div className="h-full bg-slate-100 flex flex-col animate-fadeIn overflow-hidden">
      ...
      <div className="bg-pokeRed p-6 shadow-xl relative z-10 flex-shrink-0">
         ...
         <div className="flex gap-2 mb-3">
           {[{ id: 'dex', label: '📕 Pokédex' }, { id: 'progress', label: '🏅 Progresso' }].map(t => (
             <button key={t.id} onClick={() => setTab(t.id)}
               className={`flex-1 py-2 rounded-xl text-[11px] font-black uppercase tracking-wider transition-all ${tab === t.id ? 'bg-white text-pokeRed shadow' : 'bg-white/20 text-white/80'}`}>
               {t.label}
             </button>
           ))}
         </div>
      ...
      {tab === 'progress' && (
        <div className="flex-1 overflow-y-auto p-4 custom-scrollbar bg-slate-50">
          {(() => {
            const view = getDexProgressView(gameState);
            const claim = (id) => setGameState && setGameState(prev => claimDexReward(prev, id).state);
            const rewardText = (rw = {}) => {
              const parts = [];
              if (rw.currency) parts.push(`${rw.currency.toLocaleString()} 🪙`);
              Object.entries(rw.items || {}).forEach(([k, v]) => parts.push(`${v}× ${k}`));
              Object.entries(rw.materials || {}).forEach(([k, v]) => parts.push(`${v}× ${k}`));
              return parts.join(' · ');
            };
            const Bar = ({ pct, color = '#ef4444' }) => (
              <div className="w-full bg-slate-200 h-2.5 rounded-full overflow-hidden">
                <div className="h-full rounded-full transition-all" style={{ width: `${pct}%`, background: color }} />
              </div>
            );
            const RewardCard = ({ r }) => {
              const prog = getDexRewardProgress(gameState, r);
              const complete = isDexRewardComplete(gameState, r);
              const claimed = isDexRewardClaimed(gameState, r);
              const pct = Math.min(100, Math.round((prog / (r.target || 1)) * 100));
              return (
                <div className={`rounded-2xl p-3 border-2 ${claimed ? 'bg-emerald-50 border-emerald-200' : complete ? 'bg-amber-50 border-amber-300' : 'bg-white border-slate-100'}`}>
                  <div className="flex items-center justify-between gap-2 mb-1">
                    <p className="text-[12px] font-black text-slate-800 uppercase italic">{r.title}</p>
                    <span className="text-[10px] font-black text-slate-400">{prog}/{r.target}</span>
                  </div>
                  <Bar pct={pct} color={complete ? '#f59e0b' : '#94a3b8'} />
                  <p className="text-[9px] font-bold text-slate-500 mt-1.5">🎁 {rewardText(r.reward)}{r.border ? ' · 🖼️ Borda' : ''}</p>
                  <button
                    disabled={!complete || claimed || !setGameState}
                    onClick={() => claim(r.id)}
                    className={`mt-2 w-full py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${
                      claimed ? 'bg-emerald-100 text-emerald-600' : complete ? 'bg-amber-500 text-white active:scale-95 shadow' : 'bg-slate-100 text-slate-400'
                    }`}>
                    {claimed ? '✅ Coletado' : complete ? '🎁 Coletar' : '🔒 Em progresso'}
                  </button>
                </div>
              );
            };
              ...
              <div className="flex flex-col gap-4">
                ...
                <div className="bg-white rounded-3xl p-4 shadow-sm border border-slate-100">
                  <div className="flex items-center justify-between mb-2">
                    <p className="text-[11px] font-black uppercase tracking-widest text-slate-500">Dex Nacional</p>
                ...
                <div className="bg-white rounded-3xl p-4 shadow-sm border border-slate-100">
                  <p className="text-[11px] font-black uppercase tracking-widest text-slate-500 mb-3">Progresso por Região</p>
                  <div className="flex flex-col gap-2.5">
                    {view.regions.map(rg => (
                      <div key={rg.region}>
                        <div className="flex items-center justify-between mb-1">
                          <span className="text-[11px] font-black text-slate-700 capitalize">{rg.label}</span>
                          <span className="text-[10px] font-bold text-slate-400">{rg.have}/{rg.size} ({rg.pct}%)</span>
                        </div>
                        <Bar pct={rg.pct} color={rg.pct >= 100 ? '#16a34a' : '#38bdf8'} />
                      </div>
                    ))}
                  </div>
                </div>

                {/* Recompensas — Nacional */}
                <div>
                ...
                {/* Recompensas — Regiões */}
                <div>
                  <p className="text-[10px] font-black uppercase tracking-[0.2em] text-amber-500 mb-2 px-1">🏅 Regiões Completas</p>
                  <div className="grid grid-cols-1 gap-2.5">
                    {regionRewards.map(r => <RewardCard key={r.id} r={r} />)}
                  </div>
                </div>
              </div>
            );
