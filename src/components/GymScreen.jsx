    ...
    >
      ...
      <div className="flex justify-end pr-4 pt-12 relative z-10 pointer-events-none">
        <img
          src={gym.sprite}
          alt={gym.name}
          className="w-44 h-44 object-contain drop-shadow-[0_10px_30px_rgba(0,0,0,0.8)] scale-110 translate-y-4 group-hover:scale-125 transition-transform duration-500"
...
const EliteCard = ({ member, index, earned, locked, onClick }) => {
  ...
  return (
    <div
      className={`relative rounded-[2.5rem] overflow-hidden shadow-2xl transition-all mb-4 ${locked ? 'opacity-40 grayscale-[0.5]' : 'hover:scale-[1.02] active:scale-[0.98] cursor-pointer group'}`}
      style={{ background: `linear-gradient(165deg, ${col} 0%, ${c2} 40%, #150a0a 100%)`, minHeight: '140px' }}
...
const GymDetailModal = ({ gym, earned, locked, onChallenge, onClose }) => {
  const col = TYPE_COLOR_HEX[gym.type] || '#555';
      ...
      >
        ...
        <div className="p-8">
           ...
           <div className="mt-8 flex flex-col gap-3">
              ...
              {locked && (
                <div className="w-full bg-slate-800 text-slate-500 py-5 rounded-2xl font-black uppercase tracking-widest text-sm text-center border border-white/5">
                  🔒 Bloqueado
                </div>
              )}
           </div>
