...
export default function App() {
  ...
  const processedRoutes = useMemo(() => {
    if (!gameState.worldFlags?.includes('starters_spotted')) return ROUTES;
    
    const newRoutes = JSON.parse(JSON.stringify(ROUTES)); // Deep clone to avoid mutating original ROUTES
    const addSafe = (routeId, id, lvl) => {
      const route = newRoutes[routeId];
      if (route && route.enemies) {
        if (!route.enemies.some(e => e.id === id)) {
          route.enemies.push({ id, level: lvl });
        }
      }
    };

    // Rota 1: Charmander e Squirtle
    addSafe('route_1', 4, 4);
    addSafe('route_1', 7, 4);

    // Rota 22: Eevee (Bônus)
    addSafe('route_22', 133, 5);

    // Floresta de Viridian: Bulbasaur e Pikachu
    addSafe('viridian_forest', 1, 7);
    addSafe('viridian_forest', 25, 8);
  ...
  // ─── INTERPRETADOR DE EFEITOS DE GOLPE ──────────────────────────────────────
  ...
  const interpretMoveEffect = (move) => {
    ...
    // ── Condições de Status no inimigo ──
  ...
  const spawnEnemy = useCallback(() => {
    ...
    // ── 1. EMBOSCADA VILÃ (Chance Global reduzida para ~1% para focar em selvagens) ──
    ...
    setGameState(prev => {
      ...
      if (move.category === 'Status' || move.power === 0) {
        ...
        if (fx.noEffect) {
          ...
          // Status condition
          if (fx.statusEffect) {
            const statusNames = { burn:'🔥 Queimadura', poison:'☠️ Veneno', toxic:'☠️ Veneno Grave', sleep:'💤 Sono', paralyze:'⚡ Paralisia', confuse:'💫 Confusão', freeze:'❄️ Congelado' };
        ...
        if (enemyStatus.includes('paralyze') && Math.random() < 0.25) {
          ...
          if (enemyMove) {
            if (enemyMove.category === 'Status' || enemyMove.power === 0) {
              ...
              if (eff > 0 && eff < 1) addLog(`🛡️ Golpe de ${updatedEnemyFinal.name} não foi muito efetivo...`, 'enemy');
  ...
  const startKeyBattle = useCallback((battleData) => {
    const teamMember = battleData.team[0];
    const base = POKEDEX[teamMember.id];
    if (!base) return;

    const lvl = teamMember.level || 5;
    const maxHp = Math.ceil((base.maxHp || base.hp || 50) * 3.5 * (lvl / 20)); 
    const statScale = (lvl / 10) * 1.25;
    ...
    playBGM(battleData.category === 'legendary' ? 'legendary' : 'rival');
  ...
  const handleChallengeGym = useCallback((gymData) => {
    ...
    const maxHp = Math.ceil((base.maxHp || base.hp || 50) * 3.5 * (lvl / 20)); // Aumentado de 2.5x para 3.5x
    ...
    setGameState(prev => {
      ...
      const newTeam = prev.team.map((p, i) => {
        ...
        if (newXp >= xpNeeded) {
          ...
          // Stats: floor((floor((2 * base * level) / 100) + 5) * shinyMult)
    ...
    <div className={`app-shell ${gameState.settings?.displayMode === 'pc' ? 'pc-mode' : ''}`}>
      ...
      {(!loading && user) && (
        ...
        >
          ...
          <button onClick={handleGoToCity} className={`flex flex-col items-center justify-center py-2 transition-all ${currentView === 'city' ? 'text-indigo-500 scale-110' : 'text-slate-400 opacity-60'}`}>
