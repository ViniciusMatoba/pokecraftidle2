...
const cleanBattleText = (value) => {
  ...
  return value
    .replace(/íÂ¢í¢â‚¬â€œ¼|íÂ¢í¢â‚¬â€œ½|â–¼/g, 'DOWN')
    .replace(/íÂ¢í¢â‚¬â€œ²|â–²/g, 'UP')
    .replace(/í°Å¸â€™Å /g, 'Auto-Pocao')
    .replace(/í°Å¸â€™Â«|ðŸ’«/g, 'Confusao')
    .replace(/í°Å¸â€™Â¤|ðŸ’¤/g, 'Sono')
    .replace(/í°Å¸â€™Â¥|ðŸ’¥/g, 'Dano')
    .replace(/í°Å¸â€Â¥|ðŸ”¥/g, 'Queimadura')
    .replace(/⚡/g, 'Paralisia')
    .replace(/✨/g, 'Shiny')
    .replace(/í¢ËœÂ/g, '')
    .replace(/é/g, 'e')
...
export default function App() {
  ...
  // ─Â─Â─Â UNIFICAÇíO DE COLEÇíO ─Â─Â─Â─Â─Â─Â─Â─Â─Â─Â─Â─Â─Â─Â─Â─Â─Â─Â─Â─Â─Â─Â─Â─Â─Â─Â─Â─Â─Â─Â─Â─Â─Â─Â─Â─Â─Â─Â─Â─Â─Â─Â─Â─Â─Â─Â─Â─Â─Â─Â
  ...
  const spawnEnemy = useCallback(() => {
    ...
    if (Math.random() < 0.01 && route.type === 'farm') {
      ...
      const baseMult = 1.1; // Vilï¿½es sï¿½o um pouco mais perigosos
    ...
    if (hasTrainers && Math.random() < effectiveTrainerChance) {
      ...
      const baseMult = 1.05; // Treinadores comuns sï¿½o levemente mais fortes que selvagens
      ...
      setCurrentEnemy({
        ...trainerPoke,
        name: trainerPoke.name || 'PokÃ©mon',
    ...
    setGameState(prev => {
      ...
      if (move.category === 'Status' || move.power === 0) {
        ...
        if (fx.noEffect) {
          ...
          // Status condition
          if (fx.statusEffect) {
            const statusNames = { burn:'🔥 Queimadura', poison:'🧪 Veneno', toxic:'🧪 Veneno Grave', sleep:'💤 Sono', paralyze:'⚡ Paralisia', confuse:'💫 Confusão', freeze:'❄️ Congelado' };
      ...
      // ⛏️” PROTECTED: Sistema de Exaustão — NíO EDITAR SEM AUTORIZAÇíO EXPLíCITA
  ...
  useEffect(() => {
    ...
    setGameState(prev => {
      ...
      if (currentEnemy.badgeToGive && !newBadges.includes(currentEnemy.badgeToGive)) {
        ...
        addLog(`íÂ¢â‚¬Â¦ Recebeu a Insígnia: ${currentEnemy.badgeToGive.replace(/_/g, ' ')}!`, 'system');
        sfxGym();
                ...
                <div className="p-5 flex-1 flex flex-col overflow-hidden">
                   ...
                   <div className="flex flex-col gap-3 overflow-y-auto pr-1 custom-scrollbar flex-1 pb-4">
                      {[
                        ...
                        const badgeMap = { boulder_badge: 1, cascade_badge: 2, thunder_badge: 3, rainbow_badge: 4, soul_badge: 5, marsh_badge: 6, volcano_badge: 7, earth_badge: 8 };
                        const badgeId = badgeMap[item.availableFrom];
                        return badgeId ? (gameState.badges || []).includes(badgeId) : (gameState.worldFlags || []).includes(item.availableFrom);
                      }).map(item => {
                   ...
                   <div className="flex gap-2 overflow-x-auto pb-3 mb-3">
                      {Object.keys(CRAFTING_RECIPES).map(category => (
                        <button
                          ...
                          className={`shrink-0 px-4 py-2 rounded-xl text-[10px] font-black uppercase ${forgeCategory === category ? 'bg-slate-800 text-white' : 'bg-slate-100 text-slate-500'}`}
                        >
                   ...
                   <div className="space-y-6 overflow-y-auto pr-1 custom-scrollbar flex-1 pb-6">
                      {Object.entries(CRAFTING_RECIPES).filter(([category]) => category === forgeCategory).map(([category, items]) => (
                        <div key={category} className="space-y-3">
                           ...
                           <div className="flex flex-col gap-3">
                              {items
                                .filter(item => hasForgeRecipe(gameState, item.id))
