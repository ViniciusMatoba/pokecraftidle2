...
function recoverString(str) {
    let current = str;
    // Iteratively decode until no more mangled characters are found
    // or until it starts producing replacement characters
    for (let i = 0; i < 5; i++) {
        if (!/[âÃíìðŸ]/.test(current)) break;
        try {
...
walkDir(targetDir, (filePath) => {
  ...
  if (changed) {
    ...
    console.log(`Master Repaired: ${filePath}`);
  }
});
